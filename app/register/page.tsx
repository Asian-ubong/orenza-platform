...
export default function RegisterPage() {
  ...
  async function register(event: FormEvent) {
    ...
    try {
      ...
      // The server has already created and authenticated the account. Install
      // the returned session directly; do not perform a second sign-in request.
  ...
  return (
    <main className="authCanvas">
      <section className="authCard" style={{ maxWidth: 620 }}>
        ...
        <h1>Join ORENZA</h1>
        <p className="authSub">Create your account in one step. After registration, you will enter your one-time promotion code by scanning the QR code or typing it manually.</p>

        <div className="authNotice"><LockKeyhole size={17}/><span>No email OTP. No email verification screen. Register once, receive your secure ORENZA session, then continue to promotion access.</span></div>
        ...
        <form onSubmit={register} className="authForm">
          ...
          <div style={{ display: 'grid', gap: 6, fontSize: 12, color: '#5d655f', marginTop: 2 }}>
            ...
            <span style={{ display: 'flex', gap: 7, alignItems: 'center' }}><Check size={14}/> You continue directly to the one-time promotion code</span>
          </div>
