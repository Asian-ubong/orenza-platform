    ...
    <main className="splash authLanding" aria-label="Orenza welcome">
      ...
      <div
        ...
        style={{
          ...
          transform: revealed ? 'translateY(0) scale(1)' : 'translateY(14px) scale(.97)',
          transition: 'opacity .7s ease, transform .7s ease',
        }}
      >
        <img
          className="brandHeroLogo"
          style={{ width: 'min(360px,100%)', height: 'auto', display: 'block', margin: '0 auto 22px' }}
          src="/brand/orenza-mark.svg"
          alt="ORENZA"
        />
        ...
        <div style={{ marginTop: 24, padding: 18, border: '1px solid rgba(201,160,99,.45)', borderRadius: 16, background: 'rgba(201,160,99,.08)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9, fontWeight: 800 }}>
            <QrCode size={20} /> ORENZA ACCESS SENSOR
          </div>
          <p style={{ opacity: .78, fontSize: 12, lineHeight: 1.5, margin: '9px 0 13px' }}>
            Scan an approved Orenza invitation to continue to account login or registration. The private promo terminal is not shown until authenticated tester access is authorized.
          </p>
          <button type="button" className="btn full" onClick={scanInvitation} disabled={scanning}>
            {scanning ? <><Camera size={17} /> Scanning…</> : <><Camera size={17} /> Scan Orenza invitation</>}
          </button>
          {scanning && (
            <div style={{ marginTop: 12 }}>
              <video ref={videoRef} autoPlay playsInline muted style={{ width: '100%', maxHeight: 280, objectFit: 'cover', borderRadius: 12, background: '#05080c' }} />
              <button type="button" className="textButton" onClick={stopScanner}><X size={15} /> Stop scanner</button>
            </div>
          )}
