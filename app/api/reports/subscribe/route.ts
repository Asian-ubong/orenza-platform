...
export async function POST(req: Request) {
  try {
    ...
    const result = await sendOrenzaEmail({
      to: email,
      subject: 'Confirm your ORENZA operational reports',
    ...
    if (!result.configured) return NextResponse.json({ error: 'Subscription was saved, but SMTP2GO email delivery is not configured. Configure the ORENZA SMTP2GO transport, then subscribe again.' }, { status: 503 });
    if (!result.ok) return NextResponse.json({ error: 'Subscription was saved, but the confirmation email could not be delivered. Check the SMTP2GO sender and SMTP credentials, then try again.' }, { status: 503 });
