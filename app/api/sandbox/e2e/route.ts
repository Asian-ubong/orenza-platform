...
export async function POST(request: Request) {
  try {
    ...
    const allocation = await db.rpc('orenza_issue_sandbox_welcome_bonus', {
      ...
      p_amount: 5000,
      p_currency: 'USD',
      p_idempotency_key: `${user.id}:welcome:5000`,
    });
    ...
    const settlement = await db.rpc('orenza_sandbox_settle_trade', {
      p_user_id: user.id,
      p_sandbox_order_id: orderId,
      p_provider: 'ORENZA_SANDBOX',
      p_environment: 'SANDBOX',
      p_external_trade_id: requestId,
      p_realized_pnl: pnl,
      p_returned_capital: 100,
      p_provider_reconciled: true,
      p_reconciliation_reference: `RECON-${requestId}`,
    ...
    const eligibility = await db.rpc('orenza_check_profit_eligibility', {
    ...
    if (eligible && pnl > 0) {
      const payoutResult = await db.rpc('orenza_request_profit_payout', {
        ...
        p_currency: 'USD',
        p_country_code: null,
        p_payout_method: 'PAYSTACK_TEST_SIMULATION',
        p_provider_adapter: 'PAYSTACK_TEST',
        p_request_id: `${requestId}:PAYOUT`,
      });
    ...
    return Response.json({
      ok: true,
      mode: 'SANDBOX',
      real_money_order_submitted: false,
      steps: {
        authentication: 'PASS',
        welcome_allocation: 'PASS',
        sandbox_trade: 'PASS',
        settlement: 'PASS',
        pnl: pnl,
        ...
        ledger_audit: 'RECORDED_BY_BACKEND',
        paystack_test: payout ? 'REQUEST_RECORDED_AS_PAYSTACK_TEST' : 'NOT_REACHED',
      },
