...
begin
  ...
  from public.orenza_tester_invites as ti
  where ti.code_hash = lower(trim(p_code_hash))
    and ti.active = true
  ...
  if found and v_access.active = true and v_access.expires_at > now() then
    ...
    set access_token_hash = lower(trim(p_access_token_hash))
  ...
  if found then
    ...
    values(p_user_id, v_invite.id, lower(trim(p_access_token_hash)), now(), v_expires, true);
  end if;
...
-- Keep the production pilot invitation enabled without storing the plaintext code in source.
update public.orenza_tester_invites
set active = true
where code_hash = '29208f1bdd5f1818c8fc9dcafb1141366664c79db9e36906b9bbfbe9dd0aa6bb'
  and starts_at <= now()
  and expires_at > now();
