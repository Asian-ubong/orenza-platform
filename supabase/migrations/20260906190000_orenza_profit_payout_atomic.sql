...
begin
  ...
  where user_id = p_user_id and currency = upper(trim(p_currency))
  for update;
  ...
  set payout_reserved_profit = coalesce(payout_reserved_profit, 0) + p_amount,
      updated_at = now()
  where id = v_wallet.id;

  insert into public.orenza_payout_requests (
    id, user_id, wallet_id, payout_id, request_id, amount, currency,
    payout_method, provider_adapter, status, metadata
  ) values (
    v_id, p_user_id, v_wallet.id, v_payout_id, v_request_id, p_amount,
    upper(trim(p_currency)), upper(trim(p_payout_method)), upper(trim(p_provider_adapter)),
    'PENDING', jsonb_build_object('source', 'SANDBOX_PROFIT', 'execution', 'DISABLED')
  );

  return query
  select p.id, p.payout_id, p.request_id, p.amount, p.currency,
         p.payout_method, p.provider_adapter, p.status, p.created_at,
         greatest(coalesce(w.withdrawable_profit, 0) - coalesce(w.payout_reserved_profit, 0), 0)
  from public.orenza_payout_requests p
  join public.sandbox_wallets w on w.id = p.wallet_id
  where p.id = v_id;
end;
$$;

create or replace function public.orenza_decide_profit_payout(
  p_payout_id uuid,
...
begin
  if p_payout_id is null or p_actor_user_id is null then
  ...
  from public.orenza_payout_requests
  ...
  set status = v_next,
      failed_at = case when v_next = 'REJECTED' then now() else failed_at end
  where id = v_payout.id;

  if v_decision = 'DECLINED' and v_payout.wallet_id is not null then
    update public.sandbox_wallets
    set payout_reserved_profit = greatest(coalesce(payout_reserved_profit, 0) - v_payout.amount, 0),
        updated_at = now()
    where id = v_payout.wallet_id;
  end if;

  insert into public.orenza_approval_audit (
    target_type, target_id, decision, actor_user_id,
    previous_status, new_status, reason
  ) values (
    'PAYOUT', v_payout.id, v_decision, p_actor_user_id,
    'PENDING', v_next, v_reason
  );
