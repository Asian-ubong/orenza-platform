create or replace function public.orenza_create_profit_payout(p_user_id uuid,p_amount numeric,p_currency text default 'USD',p_payout_method text default 'MANUAL',p_provider_adapter text default 'SANDBOX_DISABLED')
...
as $$
declare v_wallet public.sandbox_wallets%rowtype; v_id uuid; v_payout_id text; v_request_id text; v_available numeric;
begin
 ...
 v_id:=gen_random_uuid(); v_payout_id:='PAY-'||upper(substr(replace(v_id::text,'-',''),1,12)); v_request_id:='REQ-'||upper(substr(replace(gen_random_uuid()::text,'-',''),1,12));
 ...
 insert into public.orenza_payout_requests(id,user_id,wallet_id,payout_id,request_id,amount,currency,payout_method,provider_adapter,status,metadata) values(v_id,p_user_id,v_wallet.id,v_payout_id,v_request_id,p_amount,upper(trim(p_currency)),upper(trim(p_payout_method)),upper(trim(p_provider_adapter)),'PENDING',jsonb_build_object('source','SANDBOX_PROFIT','execution','DISABLED'));
...
language plpgsql security definer set search_path=public
as $$
declare v_payout public.orenza_payout_requests%rowtype; v_next text; v_decision text:=upper(trim(coalesce(p_decision,''))); v_reason text:=nullif(left(trim(coalesce(p_reason,'')),500),'');
begin
 ...
 select pr.* into v_payout from public.orenza_payout_requests pr where pr.id=p_payout_id for update;
 ...
 update public.orenza_payout_requests pr set status=v_next,failed_at=case when v_next='REJECTED' then now() else pr.failed_at end where pr.id=v_payout.id;
 if v_decision='DECLINED' and v_payout.wallet_id is not null then update public.sandbox_wallets sw set payout_reserved_profit=greatest(coalesce(sw.payout_reserved_profit,0)-v_payout.amount,0),updated_at=now() where sw.id=v_payout.wallet_id; end if;
